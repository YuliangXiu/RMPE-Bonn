...
def test_parametric_pose_NMS(delta1,delta2,mu,gamma):
    ...
    #prepare data
    h5file = h5py.File("./preds/bonn-test.h5",'r')
    preds = np.array(h5file['preds'])
    scores = np.array(h5file['scores'])    
    indexs = [line.rstrip(' ').rstrip('\r').rstrip('\n') for line in open("annot/bonn-test0.09/index.txt")]
    scores_proposals = np.loadtxt("annot/bonn-test0.09/score.txt")
    
    #get bounding box sizes    
    bbox_file = h5py.File("annot/bonn-test0.09/test-bbox.h5",'r')
