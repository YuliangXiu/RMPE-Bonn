...
local function parse(arg)
    ...
    cmd:option('-trainIters',     370000, 'Number of train iterations per epoch')
    cmd:option('-iterSize',            1, 'Global batch size = iterSize * trainBatch')
    cmd:option('-trainBatch',          5, 'Mini-batch size')
    cmd:option('-validIters',       8374, 'Number of validation iterations per epoch')
    cmd:option('-validBatch',          1, 'Mini-batch size for validation')
    cmd:option('-nValidImgs',       8374, 'Number of images to use for validation')
    cmd:text()
