...
local function parse(arg)
    ...
    cmd:text()
    cmd:option('-expID',       'finetune-PyraNet', 'Experiment ID')
    cmd:option('-dataset',      'ai-cha', 'Dataset choice: mpii-cs | mpii-box | coco')
    -- cmd:option('-dataset',  'bonn-train', 'Dataset choice: mpii-cs | mpii-box | coco')
    ...
    cmd:option('-nGPU',                2, 'Default GPUs number')
    ...
    cmd:option('-parallelSPPE',   'MPII_4stacked.t7', 'Provide full path to a parallel SPPE')
    cmd:text()
    cmd:text(' ---------- Model options --------------------------------------')
    cmd:text()
    cmd:option('-netType',          'hg', 'Options: hg | hg-stacked')
    cmd:option('-loadModel',      'final_model_ai-cha-pyranet.t7.t7', 'Provide full path to a previously trained model')
    -- cmd:option('-loadModel',      'mpii_base_14.t7', 'Provide full path to a previously trained model')
    ...
    cmd:option('-nParaStack',          4, 'Number of hourglasses of parallel SPPE')
    ...
    -- cmd:option('-trainIters',       2800, 'Number of train iterations per epoch')
    -- cmd:option('-iterSize',            1, 'Global batch size = iterSize * trainBatch')
    -- cmd:option('-trainBatch',          5, 'Mini-batch size')
    -- cmd:option('-validIters',        558, 'Number of validation iterations per epoch')
    -- cmd:option('-validBatch',          1, 'Mini-batch size for validation')
    -- cmd:option('-nValidImgs',        558, 'Number of images to use for validation')
    -- ai-cha
    cmd:option('-nEpochs',             5, 'Total number of epochs to run')
    cmd:option('-trainIters',      74000, 'Number of train iterations per epoch')
    cmd:option('-iterSize',            1, 'Global batch size = iterSize * trainBatch')
    cmd:option('-trainBatch',          5, 'Mini-batch size')
    cmd:option('-validIters',       4181, 'Number of validation iterations per epoch')
    cmd:option('-validBatch',          2, 'Mini-batch size for validation')
    cmd:option('-nValidImgs',       8362, 'Number of images to use for validation')
    -- -- mpii-box
    -- cmd:option('-nEpochs',             5, 'Total number of epochs to run')
    -- cmd:option('-trainIters',       8000, 'Number of train iterations per epoch')
    -- cmd:option('-iterSize',            1, 'Global batch size = iterSize * trainBatch')
    -- cmd:option('-trainBatch',          5, 'Mini-batch size')
    -- cmd:option('-validIters',       3000, 'Number of validation iterations per epoch')
    -- cmd:option('-validBatch',          1, 'Mini-batch size for validation')
...
if opt.GPU == -1 then
    ...
    require 'cudnn'
    nnlib = cudnn
    cutorch.setDevice(opt.GPU)idxs, preds, hms, inp = loadPreds('ai-cha/finetune-PyraNet/preds', true, false)
end

if true or opt.addSSTN or opt.addParallelSPPE then
    require 'stn'
