...
function transformBox(pt, ul, br, res)
...
function transformBoxPreds(coords, ul, br, res)
    local origDims = coords:size()
    coords = coords:view(-1,2)
    local newCoords = coords:clone()
    for i = 1,coords:size(1) do
        newCoords[i] = transformBoxInvert(coords[i], ul, br, res)
    end
    return newCoords:view(origDims)
end
...
function cropBox(img, ul, br, rot, res)
    if br[1] <= ul[1]+5 then br[1] = ul[1]+5 end
    if br[2] <= ul[2]+5 then br[2] = ul[2]+5 end
